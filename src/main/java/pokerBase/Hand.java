...
public class Hand {
	...
	public void EvalHand() {
		...
		// Three of a Kind
			...
			ScoreHand(eHandStrength.Pair,CardsInHand.get(eCardNo.FirstCard.getCardNo()).getRank().getRank(), 0, 0);
		}
		//TODO: You need to build the logic to figure out Three of a Kind
		
		// Two Pair
		//else if (Pair ==true && ) {
		//	ScoreHand(eHandStrength.TwoPair,CardsInHand.get(eCardNo.FirstCard.getCardNo()).getRank().getRank(), 0, 0);
		else if (CardsInHand.get(eCardNo.FirstCard.getCardNo()).getRank() == CardsInHand
